...
from tiny_model.tokenization.tokenization import enc, dec
...
class TinyModel(nn.Module):
    ...
    def __getitem__(self, mlp_tag):
        ...
        tag_pat = '^(?P<mlp_type>M|Rm|Ra|A|Mo)(?P<layer>\d+)((?:\D)(?P<feature_idx>\d+))?$'
        match = re.match(tag_pat, mlp_tag)

        if match:
            mlp_type, layer, feature_idx = match.group('mlp_type'), match.group('layer'), match.group('feature_idx')
        ...
        if mlp_tag not in self.sparse_mlps:
            if mlp_tag in DEFAULT_SPARSE_MLPS:
                self.sparse_mlps[mlp_tag] = SparseMLP.from_pretrained(DEFAULT_SPARSE_MLPS[mlp_tag])
            else:
                assert False, dedent('''
